...
Benchmarks on **NVIDIA RTX A5000** (24 GB, compute capability 8.6).

### Aerial Explosion (3 grids — density, emission, temperature)

| Resolution | Mode | Time |
| --- | --- | --- |
| 496 × 577 | Flat | 551 ms |
| 4096 × 2160 | Deep (80 samples) | 2830 ms |

Deep output at 4K UHD with 80 max samples/pixel (avg 5.8) fits within 4050 MB GPU allocation. Ray march kernel dominates at 87–99% of total time; FP16→FP32 conversion and CPU readback are negligible at proxy resolution and ~12% at 4K deep due to the larger data transfer.

### WDAS Cloud (1 grid — density only, HDRI-lit)

| Resolution | Mode | Time |
| --- | --- | --- |
| 1276 × 770 | Flat + HDRI SH | 3687 ms |

The WDAS cloud is a large, dense volume (bbox 718) with full HDRI spherical harmonics lighting. The longer render time compared to the explosion reflects the higher optical density requiring more samples per ray.
